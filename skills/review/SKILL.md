...
**Resultado esperado:** Um relatório de auditoria técnica completo emitido como artefato interativo e persistido no vault.
...
3. **Emissão do Relatório**:
   ...
   * **Vault**: Salve em `10-review-reports/{projeto}_{YYYY-MM-DD}_[descrição]_[tipo].md`.
...
2. Processe cada item iterativamente: aplique a menor correção possível, atualize o `task.md` e ambos os relatórios (artefato IDE e cópia no vault) com a resolução.
3. Ao concluir, apresente o comando de teste e sugira o próximo domínio da esteira.
...
- [ ] O template do domínio correto foi utilizado?
- [ ] O relatório foi persistido tanto como artefato interativo quanto no vault?
- [ ] Cada item corrigido possui a resolução documentada no relatório?
- [ ] A suíte de testes continua passando 100% verde após as correções?
- [ ] Foi sugerido o próximo domínio da esteira de qualidade?
