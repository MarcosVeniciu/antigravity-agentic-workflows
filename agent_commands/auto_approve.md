...
- `git --no-pager log $(git describe --tags --abbrev=0)..HEAD --oneline`
  - **Purpose:** Used by the `/release` workflow to extract all commits since the last tag for semantic versioning calculation and release notes generation.
- `git branch --show-current`
  - **Purpose:** Used by the `/release` workflow to determine the current branch as part of the release candidate plan.
- `git --no-pager log --oneline`
  - **Purpose:** Used by the `/release` workflow as a fallback to extract all commits when the repository does not have any tags yet.
