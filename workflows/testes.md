...
1. **Read the artifacts:** Review the implementation plan and diagrams generated by `/artefatos` to understand the exact scope, interfaces, and acceptance criteria.
...
| **Exception Paths** | Error conditions, timeouts, unauthorized access. | External service down → raises ServiceUnavailableError. |
...
* **🚫 NO PRODUCTION CODE:** Do absolutely NOT write, modify, or generate any production/business logic files. Your scope is exclusively test files.
...
> **[NEXT STEP]** ➡️ Once the tests are generated, output:
> *"🔴 Test suite created. Performance profiling is embedded. Execute `/codigo` to implement the production logic."*
