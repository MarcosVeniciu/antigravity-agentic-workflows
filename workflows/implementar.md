...
description: "Executes Phase 2 of development unifying batch planning, TDD (Red + Green), and artifact synchronization."
---

# Workflow: Unified Implementation Engineer (`/implementar`)

This workflow orchestrates the technical implementation of features in a clean chat, consuming Phase 1 contracts (BDD and SDD), organizing tasks by Context Batches, and executing the TDD cycle (Red ➔ Green) with atomic state updates.

---

## 🎯 Workflow Steps

### Step 1: Bootstrapping & Contract Reading (Clean Chat)
...
3. Emit the interactive tracking `task_list.md` artifact using template `resources/task_template.md` from the `@implementar-plan` skill.
   ...
   * Set the final phase of the checklist as `Phase N: Execute Test Suite and Fine-Tuning`.
4. Configure artifact metadata with `RequestFeedback: true` and wait for developer confirmation before modifying code.

---

### Step 3: Atomic Execution by Context Batch (Red ➔ Green in the Same Call)
...
1. Upon reaching `Phase N` of `task_list.md`, provide the developer with the command to execute the complete test suite (e.g., `pytest -v -s tests/`).
2. **Self-Correction and Rollback Mechanism (Double-Strike Rule):**
   * If tests fail, attempt to fix production code (maximum of 2 consecutive attempts in the same session).
   * If the error persists after 2 attempts, **halt execution**, execute `git reset --hard HEAD` (Mode 3 of `@git` skill) to restore stable state, and instruct the user to open a new clean chat invoking `/testar` or `/debug`.
...
> **[NEXT STEP]** ➡️ *"⚙️ Phased TDD implementation completed successfully with test suite 100% green! It is highly recommended to open a new chat for structural refactoring without context bias. Execute `/refatorar` to start code cleanup and improvement."*